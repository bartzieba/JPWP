...
public class KontoGUI implements Obserwator {
    ...
    public KontoGUI(User l_user){
        ...
        generujPDFButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            String input=JOptionPane.showInputDialog("Podaj scieżkę do folderu w którym wygrnerować pdf:");
            String path = input;
            PDF nowypdf=new PDF(kwota_wiersz,tytul_wiersz,dane_nadawcy_wiersz,adres_nadawcy_wiersz,data_wiersz,nr_konta_nadawcy_wiersz,path);
            nowypdf.stworzPDF();
            }
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(table1.getRowCount()>0) {
                    for (int i = 0; i < table1.getRowCount(); i++) {
                        if (table1.isRowSelected(i)) wiadomosc_textPane.setText(table1.getValueAt(i, 0).toString());
                    }
                }
            }
        });
    ...
    public void wiadomosci()throws SQLException,ClassNotFoundException{
        PreparedStatement pst=MysqlConnection.Connect().prepareStatement("select tresc,data from wiadomosci");
        ResultSet rs=pst.executeQuery();
