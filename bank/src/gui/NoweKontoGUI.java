...
public class NoweKontoGUI {
    ...
    public void stworzKonto(){
        ...
        if (user_haslo.matches("(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{8,}")==false){
        ...
        }else if (user_kodPocztowy.matches("^\\d{2}-\\d{3}$")==false) {
            JOptionPane.showMessageDialog(null,"Niepoprawny format kodu pocztowego","Błąd",JOptionPane.ERROR_MESSAGE);
        }else if (user_email.matches("^[A-Za-z0-9+_.-]+@(.+)$")==false) {
            JOptionPane.showMessageDialog(null,"Niepoprawny format adresu email","Błąd",JOptionPane.ERROR_MESSAGE);
        }else
         {
            try {
